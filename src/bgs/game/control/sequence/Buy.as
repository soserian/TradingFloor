package bgs.game.control.sequence
{
	...
	public class Buy extends SequenceBase
	{
		...
		private function _exeSelPop():void
		{
			...
			_main.popupMc.gotoAndStop(Game.BUY_SEL_POPUP_FRAME);
			
			_selectView = new SelStockPop(_main.popupMc);
			_selectView.addEventListener(BuyEvent.BUY_SELECT_COMPLETE, _handleBuyCom);
		}
		
		private function _exeRulePop():void
		{
			_main.popupMc.visible = true;
			_main.popupMc.gotoAndStop(Game.RULE_VIEW_POPUP_FRAME);
		...
		private function _exeChangeStock():void
		{
			...
			_changeStockView.start(_addStepStock, _tempStock, ChangeStockPop.BUY_MODE);
		...
		private function _acceptStock():void
		{
			...
			PlayerControl.getInstance().acceptBuyStock();
...
//			setTimeout(_nextStep, Game.TEXT_BALLOON_TURM);
		...
		override protected function _setStep():void
		{
			...
			if (GameModel.getInstance().isFirst == false){
		...
		private function _handleBuyCom(e:BuyEvent):void
		{
			var vo:StockChangeVO = PlayerControl.getInstance().buy(e.data as Vector.<uint>);
			_addStepStock = vo.moves;
			_tempStock = vo.nums;
			_main.popupMc.gotoAndStop(Game.BLANK_POPUP_FRAME);
			_main.popupMc.visible = false;
			_selectView.removeEventListener(BuyEvent.BUY_SELECT_COMPLETE, _handleBuyCom);
			_selectView = null;
			_nextStep();
		}
	}
